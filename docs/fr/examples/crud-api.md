...
Une API REST complète et testée pour gérer des utilisateurs — illustrant la DI, le routage par convention, la validation des requêtes et la gestion des erreurs.
...
func userID(ctx web.Context) (int, error) {
    ...
    if err != nil || id < 1 {
        return 0, web.NewRequestError(http.StatusBadRequest, "id utilisateur invalide")
    }
    return id, nil
}
...
# HTTP 404
# {"error":{"type":"NotFound","message":"utilisateur introuvable"}}
```
...
func TestUsersCRUD(t *testing.T) {
    ...
    if resp.StatusCode != http.StatusNoContent {
        t.Fatalf("204 attendu, obtenu %d", resp.StatusCode)
    }

    // Introuvable
    resp, _ = server.ServeHTTP(newRequest("GET", "/users/1", ""))
    if resp.StatusCode != http.StatusNotFound {
        t.Fatalf("404 attendu, obtenu %d", resp.StatusCode)
    }
}
