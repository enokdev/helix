...
func TestDataLayerGuideDocumentsCoreConcepts(t *testing.T) {
	const guidePath = "docs/data-layer.md"
	if _, err := os.Stat(guidePath); err != nil {
		t.Skipf("test requires %s — guide not found: %v", guidePath, err)
	}
	guide := readTextFile(t, guidePath)
	...
	required := []string{
		...
		"## Sommaire",
		"## Modele mental",
		"`data.Repository[T, ID, TX]`",
		"`context.Context`",
		"`FindAll(ctx)`",
		"`FindByID(ctx, id)`",
		"`FindWhere(ctx, filter)`",
		"`Save(ctx, entity)`",
		"`Delete(ctx, id)`",
		"`Paginate(ctx, page, size)`",
		"`WithTransaction(tx)`",
		"`data.ErrRecordNotFound`",
		"`data.ErrInvalidFilter`",
		"## Adaptateur GORM",
		"`datagorm.OpenSQLite`",
		"`datagorm.NewRepository`",
		...
		"## Transactions",
		"`data.Transaction[TX]`",
		"`data.TransactionManager[TX]`",
		"`data.ContextWithTransaction`",
		"`data.TransactionFromContext`",
		"`//helix:transactional`",
		"`NewUserServiceTransactional(target, txManager)`",
	...
	for _, want := range required {
		if !strings.Contains(guide, want) {
			t.Fatalf("docs/data-layer.md should contain %q", want)
		}
	}
}
